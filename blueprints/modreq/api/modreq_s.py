...
from ..modreq_model import ModReqS
import re
...
def get_modreqs(uid=None, status=None, username=None):
    ...
    modreqs_data = ModReqS.objects(**query)
...
class ModReq_S(Resource):
    ...
    def validate_get(self, args):
        ...
        if not ([args.get("playerName"), args.get("status"), args.get("id")]):
            return {'error': [{"message": "an id, a status, or a playerName must be provided"}]}

    @require_api_key(access_tokens=['modreq.s'])
    def get(self):
    ...
    def post(self):
        ...
        modreq = ModReqS(username=username, request=request, location=location, status="open").save()

        return {'modreq': construct_modreq_data(modreq)}


class ModReq_S_Claim(Resource):
    post_parser = RequestParser()
    ...
    def post(self):
        ...
        modreq = ModReqS.objects(uid=uid).first()
...
class ModReq_S_UnClaim(Resource):
    post_parser = RequestParser()
    post_parser.add_argument("id", type=int, required=True)
...
rest_api.add_resource(ModReq_S, '/modreq/s')
rest_api.add_resource(ModReq_S_Claim, '/modreq/s/claim')
rest_api.add_resource(ModReq_S_UnClaim, '/modreq/s/unclaim')
rest_api.add_resource(ModReq_S_Done, '/modreq/s/done')

register_api_access_token("modreq.s", permission="api.modreq.s")

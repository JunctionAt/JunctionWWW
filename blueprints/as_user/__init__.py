...
from blueprints.roles import provide_roles
...
def switch_user(player, ext):
    try:
        ...
        to_user = None
        try:
            original_user = User.objects(name=session['original_user']).first()
            if not provide_roles(Identity(original_user.name)).can(Permission(RoleNeed('as_user'))):
            ...
            with Permission(RoleNeed('as_user')).require():
                ...
                if not to_user: to_user = User.objects(name=player).first()
